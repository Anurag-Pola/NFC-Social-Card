#!/usr/bin/env python3
"""
Build the QR mark carried on the Serene Resort & Convention card.

The QR encodes the *same* smart URL the NFC chip carries, so a scan and a tap
land on the identical hosted landing card. Change PAYLOAD here and re-run —
nothing else in the design references the URL.
...
Output: assets/serene/serene-qr.svg
...
Run:  python3 assets/serene/build-serene-qr.py
"""
import pathlib
...
PAYLOAD = "https://sereneresort.in"
DARK = "#17140f"  # the design's ink black


def main():
    qr = segno.make(PAYLOAD, error="m")
    dest = HERE / "serene-qr.svg"
    qr.save(
        dest,
        kind="svg",
        scale=1,
        ...
        lineclass=None,
        omitsize=False,
        title="Serene Resort & Convention — scan to save contact",
    )
    size = qr.symbol_size(scale=1, border=0)[0]
    print(f"serene-qr.svg  version {qr.version} · {size}x{size} modules · {PAYLOAD}")
